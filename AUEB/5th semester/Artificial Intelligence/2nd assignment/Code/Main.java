...
public class Main{
	public static void main(String[] args){
		...
		try {
			...
			System.out.print("How many times would you like the Random Forest algorithm to be executed?: ");
			...
			int[][] trainBinaryRev = new int[revCount][a]; // h grammh einai gia kritikes, h sthlh ama einai 0 den uparxei h leksi, ama einai 1 uparxei
			for (int i = 0; i < revCount; i++){
				for (int j = 0; j < a; j++) {
			...
			int posAmount = 0; //amount of positive trainReviews
			int negAmount = 0; //amount of negative trainReviews
			...
			/*for (int i = 0; i < revCount; i++){
				System.out.print(testReviews.get(i) + " :");
				for (int l = 0; l < a; l++) System.out.print(testBinaryRev[i][l]);
				System.out.println();
			}*/
			...
			nb.calculate(m, n, posAmount, negAmount, freqPos, freqNeg, testBinaryRev);
			nb.result();
			...
			RandomForest rf = new RandomForest(runTimes,trainReviews, testReviews);
			rf.calculate(m,n,runTimes,revCount, trainBinaryRev, testBinaryRev);
			rf.result(runTimes);
			/*for (int i : testReviews){
