...
class Image : public math::Array2D<Color>, image::ImageIO {
	public:
		...
		//float* pinakas = 0; // pinakas pou tha parei ta data ths ppm fwtografias kai tha ta valei sto write ths save.

		// Constructor.
		Image() {
			this->width = 0;
			this->height = 0; // ayto ginetai gia na ta diavazei swsta to WritePPM.
		...
		// dikh mou methodos gia ektypwsh twn stoixeiwn enos Vec3 (gia testing).
		void printPixel(Color pix) {
			cout << "x is: " << pix.x << ", y is: " << pix.y << ", z is: " << pix.z << endl;
		}
		...
		bool ImageIO::load(const string& filename, const string& format) {
			...
			if (format != "ppm") {
				cout << "Wrong format. Only ppm files are allowed." << endl;
				return false;
			}
		...
		bool ImageIO::save(const string& filename, const string& format) {
			...
			// case insensitive elegxos apo katw.
			if (ext == "ppm" || ext == "Ppm" || ext == "pPm" || ext == "ppM" || ext == "PpM" || ext == "pPM" || ext == "PPm" || ext == "PPM" || ext == format) {
				string path = "../../../ExampleImages/Picture.ppm";
				...
				image::WritePPM(pinakas, this->width, this->height, path2); // pairnei ta width kai height ths klashs
